...
setup_export() {
    ...
    export KERNEL_DEFCONFIG=martini_defconfig
...
setup_build_kernel_env() {
    ...
    if test ! -e ~/toolchains/clang-aosp/env_is_setup; then
      sudo pacman -S zstd tar wget curl base-devel cpio --noconfirm
...
aosp_clang() {
    if test ! -e ~/toolchains/clang-aosp/env_is_setup; then
        rm -rf ~/toolchains/clang-aosp && mkdir -p ~/toolchains/clang-aosp && cd ~/toolchains/clang-aosp
        wget https://github.com/SA9990/Toolchain/releases/download/clang-r563880c/clang-r563880c.tar.gz
        tar -C ~/toolchains/clang-aosp/ -zxvf clang-r563880c.tar.gz
        touch ~/toolchains/clang-aosp/env_is_setup
    fi
}
